...
2. Upload the entire `cobalt-bank-operations-payment-gateway` folder into:
...
1. **Extract strings to a `.pot` file**  
   ```bash
   wp i18n make-pot . i18n/cobalt-bank-operations-payment-gateway.pot --slug=cobalt-bank-operations-payment-gateway
   ```
...
2. **Initialize a new locale** (example: Spanish – `es_ES`)  
   ...
   msginit \
     --no-translator \
     --input=i18n/cobalt-bank-operations-payment-gateway.pot \
     --locale=es_ES \
     --output-file=i18n/cobalt-bank-operations-payment-gateway-es_ES.po
   ```

3. **Translate strings**  
   - Open `i18n/cobalt-bank-operations-payment-gateway-es_ES.po`  
...
4. **Compile the `.mo` file**  
   ```bash
   msgfmt \
     i18n/cobalt-bank-operations-payment-gateway-es_ES.po \
     -o i18n/cobalt-bank-operations-payment-gateway-es_ES.mo
   ```

5. **Generate JSON for React/JS blocks**  
   ```bash
   wp i18n make-json i18n i18n --domain=cobalt-bank-operations-payment-gateway
   ```
   - **Input**: All `.pot` files inside `i18n/`  
   - **Output**: `cobalt-bank-operations-payment-gateway-<locale>.json` files in `i18n/`
